...
// (2) * (1)
__device__ void
mulSE3(const float *t1, const float *q1, const float *t2, const float *q2, float *t12, float *q12) {
  q12[0] =  q2[3] * q1[0] + q2[0] * q1[3] + q2[1] * q1[2] - q2[2] * q1[1],
  q12[1] =  q2[3] * q1[1] + q2[1] * q1[3] + q2[2] * q1[0] - q2[0] * q1[2],
  q12[2] =  q2[3] * q1[2] + q2[2] * q1[3] + q2[0] * q1[1] - q2[1] * q1[0],
  q12[3] =  q2[3] * q1[3] - q2[0] * q1[0] - q2[1] * q1[1] - q2[2] * q1[2],
...
__global__ void projective_transform_kernel(
    ...
    const torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> poses_rel,
    ...
    const torch::PackedTensorAccessor32<long,1,torch::RestrictPtrTraits> cii,
    const torch::PackedTensorAccessor32<long,1,torch::RestrictPtrTraits> cjj,
    ...
    torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> bz)
{
  ...
  int cix = static_cast<int>(cii[block_id]);
  int cjx = static_cast<int>(cjj[block_id]);
  ...
  __shared__ float ti[3], tj[3], tcci[3], tccj[3], ticj[3], tcicj[3];
  __shared__ float qi[4], qj[4], qcci[4], qccj[4], qicj[4], qcicj[4];

  // load intrinsics from global memory
  if (thread_id == 0) {
    fxi = intrinsics[cix][0];
    fyi = intrinsics[cix][1];
    cxi = intrinsics[cix][2];
    cyi = intrinsics[cix][3];
  }
  if (thread_id == 1) {
    fxj = intrinsics[cjx][0];
    fyj = intrinsics[cjx][1];
    cxj = intrinsics[cjx][2];
    cyj = intrinsics[cjx][3];
  }
  ...
  if (thread_id < 3) {
    ...
    tcci[thread_id] = poses_rel[cix][thread_id];
    tccj[thread_id] = poses_rel[cjx][thread_id];
  }
  if (thread_id < 4) {
    qi[thread_id] = poses[ix][thread_id+3];
    qj[thread_id] = poses[jx][thread_id+3];
    qcci[thread_id] = poses_rel[cix][thread_id+3];
    qccj[thread_id] = poses_rel[cjx][thread_id+3];
  }

  __syncthreads();

  if (thread_id == 0) {
    mulSE3(tj, qj, tccj, qccj, tcicj, qcicj);         // Gcicj = Gccj * poses[:,jj] (Gcicj only temp)
    relSE3(ti, qi, tcicj, qcicj, ticj, qicj);         // Gicj = Gcicj * poses[:,ii].inv()
    relSE3(tcci, qcci, ticj, qicj, tcicj, qcicj);     // Gcicj = Gicj * Gcci.inv()
  }
  ...
  GPU_1D_KERNEL_LOOP(k, ht*wd) {
    ...
    const float ru = target[block_id][0][i][j] - (fxj * d * x + cxj);
    const float rv = target[block_id][1][i][j] - (fyj * d * y + cyj);
    ...
    Jz = fxj * (tcicj[0] * d - tcicj[2] * (x * d2));
    Cii[block_id][k] = wu * Jz * Jz;
    bz[block_id][k] = wu * ru * Jz;

    if (ix == jx) wu = 0; // rel transform derivative = 0 if pose_i == pose_j

    adjSE3(tccj, qccj, Jk, Jj);
    adjSE3(ticj, qicj, Jk, Ji);      // ToDo: Can optimize by calculating Ji = f(Jj)
    ...
    adjSE3(tccj, qccj, Jk, Jj);
    adjSE3(ticj, qicj, Jk, Ji);      // ToDo: Can optimize by calculating Ji = f(Jj) => Safe space for Jk
    for (int n=0; n<6; n++) Ji[n] *= -1;
    ...
    torch::PackedTensorAccessor32<float,4,torch::RestrictPtrTraits> valid)
{
  ...
  GPU_1D_KERNEL_LOOP(k, ht*wd) {
    ...
    if (Xj[2] < 0.5 * MIN_DEPTH_PROJMAP) {
      Xj[2] = 1.0;
    }
    coords[block_id][i][j][0] = fxj * (Xj[0] / Xj[2]) + cxj;
    coords[block_id][i][j][1] = fyj * (Xj[1] / Xj[2]) + cyj;

    valid[block_id][i][j][0] = (Xj[2] > MIN_DEPTH_PROJMAP) ? 1.0 : 0.0;
    ...
    torch::PackedTensorAccessor32<float,3,torch::RestrictPtrTraits> counter)
{
  ...
  if (index < ht*wd) {
    ...
    const float uj = fxj * (Xj[0] / Xj[2]) + cxj;
    const float vj = fyj * (Xj[1] / Xj[2]) + cyj;
...

{
  ...
  if (threadIdx.x == 0) {
    fx = intrinsics[block_id][0];
    fy = intrinsics[block_id][1];
    cx = intrinsics[block_id][2];
    cy = intrinsics[block_id][3];
  }
    ...
    const bool motion_only)
{
  ...
  const int num = cii.size(0);
  const int ht = disps.size(1);
  const int wd = disps.size(2);

  torch::Tensor ts = torch::arange(t0, t1).to(torch::kCUDA);
  torch::Tensor cii_exp = torch::cat({cii, cii}, 0);
  torch::Tensor jj_exp = torch::cat({ii, jj}, 0);

  std::tuple<torch::Tensor, torch::Tensor> kuniq =
    torch::_unique(cii_exp, true, true);
  ...
  for (int itr=0; itr<iterations; itr++) {
    ...
    projective_transform_kernel<<<num, THREADS>>>(
      ...
      poses_rel.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      ...
      cii.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
      cjj.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
    ...
    if (motion_only) {
      ...
      torch::Tensor C = accum_cuda(Cii, cii, kx) + eta.view({-1, ht*wd});
      torch::Tensor w = accum_cuda(wi, cii, kx);
      torch::Tensor Q = 1.0 / C;

      torch::Tensor E = torch::cat({Eii, Eij}, 0);

      SparseBlock S = schur_block(E, Q, w, jj_exp, jj_exp, kk_exp, t0, t1); // indexing??
      dx = (A - S).solve(lm, ep);

      torch::Tensor ix = jj_exp - t0;
      torch::Tensor dw = torch::zeros({ix.size(0), ht*wd}, opts);   // Should have size v!!
      ...
      dz = Q * (w - accum_cuda(dw, cii_exp, kx));
    ...
    torch::Tensor jj)
{
  ...
  torch::Tensor coords = torch::zeros({num, ht, wd, 2}, opts);
...
std::vector<torch::Tensor> schur_block_cuda(
  ...
  SparseBlock S = schur_block(E, Q, w, ii, jj, kk, t0, t1);

  std::tuple<torch::Tensor, torch::Tensor> Ab = S.get_dense();

  torch::Tensor A = std::get<0>(Ab);
  torch::Tensor b = std::get<1>(Ab);
  return {A, b};
}
